...
def get_new_fig(fn, figsize=[6, 6]):
    """Init graphics"""
...
def configcell_text_and_colors(
    ...
    if (col == (ccl - 1)) or (lin == (ccl - 1)):
        ...
        # text to ADD
        font_prop = fm.FontProperties(size=(fz-2))
        # kong-new added - starting -- this is to change the summary (last-row column text boldness)
        if col == (ccl - 2) and (lin == (ccl-1)):
            font_prop = fm.FontProperties(weight="bold",size=(fz+3))
        else:
            font_prop = fm.FontProperties( size=(fz-2))
        # kong-new added - end
        text_kwargs = dict(
        ...
        carr = [0.27, 0.30, 0.27, 0.3]
        if (col == ccl - 1) and (lin == ccl - 1):
            carr = [0.17, 0.20, 0.17, 0.3]
        ...
        if col == lin:
            ...
            facecolors[posi] = [0.35, 0.8, 0.55, 0.3]
        else:
            if col == (ccl - 2) and (lin == (ccl-1)):
                oText.set_color("r") #off-diaganal-percentage text color
            else:
                oText.set_color("k")
